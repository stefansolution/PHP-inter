...
/*** Get Stores Start Here ***/
	...
	$selectStoresql = "SELECT id,slug FROM stores";
	...
	$catStoresql = "SELECT s.slug,sc.cat_id as catid FROM `stores` s JOIN `store_categories` sc ON s.id = sc.store_id";
...
//echo "<pre>";print_r($storeCatSlugArr); die;
//echo "----------------------------------";
	...
	if ($resultCat->num_rows > 0) {
		...
		while($row = $resultCat->fetch_assoc()) {
			...
			$catId = $row['id'];		
			$catSlug = $row['slug'];
			$catPage = $web->Process("https://www.wethrift.com/tag/".$catSlug);			
			...
			foreach ($storeSlug["ids"] as $key=>$slugId) {
				$chkSlug = addslashes(trim($html->nodes[$slugId]["attrs"]["href"]));
				$chkSlug = substr($chkSlug,1);
				//"<pre>";print_r($storeSlugArr);
				//print_r($newSlug);
				//die;
				...
				if(!in_array($chkSlug,$storeSlugArr) && !in_array($chkSlug,$newSlug) ){ 
					...
					$storeSql = "INSERT INTO stores (store_name,image,domain,slug) VALUES ('".$storename."','".$storelogo."','".$storedomain."','".$storeslug."')";echo "<br/>";
					$conn->query($storeSql);
